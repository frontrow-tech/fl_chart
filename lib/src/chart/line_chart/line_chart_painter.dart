  ...
  void _drawExtraLines(CanvasWrapper canvasWrapper) {
    ...
    if (data.extraLinesData.horizontalLines.isNotEmpty) {
      for (var line in data.extraLinesData.horizontalLines) {
        if (line.width != null && line.width > viewSize.width) {
          throw 'width must be less than the view size width';
        }
        ...
        if (line.width != null) {
          if (line.position == HorizontalPosition.left) {
            from = Offset(
              leftChartPadding + (line.label?.labelWidth ?? 0),
              getPixelY(
                line.y,
                chartUsableSize,
              ),
            ...
            to = Offset(
              leftChartPadding + line.width + (line.label?.labelWidth ?? 0),
              getPixelY(
                line.y,
                chartUsableSize,
              ),
          ...
          from = Offset(leftChartPadding, getPixelY(line.y, chartUsableSize));

          to = Offset(viewSize.width - rightChartPadding,
            ...
            if (line.position == HorizontalPosition.left) {
              canvasWrapper.drawText(
                  ...
                  label.alignment.withinRect(
                    Rect.fromLTRB(
                      from.dx + padding.left - label.labelWidth,
                      from.dy - padding.bottom - tp.height + 5,
                      to.dx - padding.right - tp.width - label.labelWidth,
                      to.dy + padding.top,
                    ),
                  ));
            } else {
    ...
    if (data.extraLinesData.verticalLines.isNotEmpty) {
      for (var line in data.extraLinesData.verticalLines) {
        if (line.height != null && line.height > viewSize.height) {
          throw 'height must be less than the view size height';
        }
        ...
        if (line.height != null) {
          if (line.position == VerticalPosition.top) {
            from = Offset(
              ...
              topChartPadding + (line.label?.labelWidth ?? 0),
            );
            ...
            to = Offset(
              ...
              topChartPadding - line.height + (line.label?.labelWidth ?? 0),
            );
          } else {
            from = Offset(
              getPixelX(
          ...
          to = Offset(getPixelX(line.x, chartUsableSize),
